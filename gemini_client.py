...
# This prompt has been updated for the iterative testing workflow.
# It now asks the AI to devise a testing STRATEGY, not final payloads.
ANALYSIS_SYSTEM_PROMPT = """
You are an expert cybersecurity analyst specializing in business logic flaws.
Given a sequence of HTTP request/response conversations captured from a *single* user journey, produce only the most relevant business-logic-focused tests.

Your responsibilities:
1. **Map the Business Flow**: Summarize the exact order of endpoints touched by the captured flow using a concise ASCII diagram (e.g., `/ -> /login (POST) -> /cart`).
2. **Derive Targeted Business Tests**: For at most five critical steps in this flow, output actionable tests that stay strictly within the business logic domain. Ignore generic vulnerabilities (e.g., generic XSS/SQLi) unless they clearly stem from the captured journey.
3. **Provide Ready-to-run Commands**: Each recommended test case must include a complete `curl` command template that can be executed as-is after placeholder replacement.

Formatting & constraints:
- Each JSON entry provides the original request (method/URL/headers/body) and the full server response (status, headers, body + encoding). Use both sides of the conversation in your reasoning.
- Use placeholders `{target_domain}` (hostname only) and `{session_cookie}` (already-formatted cookie string) inside curl commands.
- Prefer HTTPS URLs and derive paths/parameters from the provided traffic only. Do not hallucinate endpoints.
- Keep commands concise, single-request payloads that demonstrate the test objective.
- Limit yourself to business logic scenarios such as state transitions, coupon/cart/order flows, authorization checks, rate/sequence controls, etc.

Return a single valid JSON object with:
{
    "business_flow_diagram": "Text diagram of the captured sequence.",
    "recommended_test_cases": [
        {
            "name": "Short name of the test",
            "description": "1-2 sentence business-logic rationale for this test",
            "curl_command": "curl ... {target_domain} ... {session_cookie}"
        }
    ]
}

If no business-logic-focused tests can be derived, return an empty `recommended_test_cases` array.
"""
...
2.  **Next Payload**: Provide the *exact*, ready-to-use `curl` command for the next attempt. Use placeholders `{target_domain}` and `{session_cookie}`. If you believe the test case is exhausted or successful, provide an empty string ("").
...
You MUST return your response as a single, valid JSON object.
{
    ...
    "next_payload": "The next curl command to execute, or an empty string.",
...
REPORTING_PROMPT = """
You are an expert cybersecurity analyst finalizing a penetration test report.
You have been provided with two pieces of information:
1.  **Initial Analysis**: An initial analysis of a web application's business logic, which includes a summary of the business flow and a list of *potential* vulnerabilities with suggested `curl` commands to test them.
2.  **Execution Results**: The actual results (STDOUT and STDERR) from running those `curl` commands.

Your task is to synthesize this information into a clear, concise, and professional report in Markdown format.

The report must include:
1.  **Executive Summary**: A brief, high-level overview of the engagement and the most critical findings.
2.  **Business Flow Overview**: A description of the application's business logic that was tested, based on the initial analysis.
3.  **Vulnerability Details**: For each vulnerability from the initial analysis:
    - State its name and description.
    - Analyze the corresponding execution results to determine if the vulnerability was **Confirmed**, **Plausible**, or **Not Confirmed**.
    - Provide a clear justification for your assessment. For **Confirmed** vulnerabilities, explain what in the STDOUT or STDERR proves the exploit was successful (e.g., "The server responded with a 200 OK and a negative balance in the JSON body, confirming price manipulation."). For **Not Confirmed** vulnerabilities, explain why the attempt failed (e.g., "The server responded with a 403 Forbidden, indicating the access control is working correctly.").
4.  **Recommendations**: Suggest concrete steps the development team should take to remediate the confirmed vulnerabilities.
5.  **Conclusion**: A final summary of the security posture of the application's business logic.

Structure the report logically with clear headings. Be professional and objective in your assessment.
"""
